...
export function LinkCard({ link, index }: { link: Link; index: number }) {
    ...
    return (
        <button
            onClick={handleClick}
            className="relative flex items-center justify-center p-1 rounded-lg bg-slate-50 hover:bg-slate-100 hover:scale-110 hover:-translate-y-1 transition-all duration-200 border border-slate-200 group w-[38px] h-[38px] cursor-pointer shadow-sm animate-slide-in opacity-0 fill-mode-forwards will-change-transform"
            style={{ animationDelay: `${index * 50}ms` }}
            title={link.name}
            aria-label={`Open ${link.name}`}
        >
            {link.isNew && (
                <span className="absolute -top-1 -right-1 flex h-[13px] w-auto px-0.5">
