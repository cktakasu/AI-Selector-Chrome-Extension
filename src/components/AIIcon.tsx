...
export const AIIcon: React.FC<AIIconProps> = React.memo(({
    link, index, isSelected,
    onOpen, onSelect, onDragStart,
}) => {
    const [iconError, setIconError] = React.useState(false);
    
    // style 計算を消去し、クラスなどをシンプルに保つ
    ...
    return (
        <div className="relative group flex flex-col items-center">
            <button
                ...
                onClick={handleClick}
                className={`relative flex flex-col items-center justify-center rounded-[13px] w-[48px] h-[48px] border bg-slate-50 hover:bg-slate-100 dark:bg-white/[0.04] dark:hover:bg-white/[0.07] active:scale-95 cursor-grab active:cursor-grabbing transition-all duration-150 ${
            ...
            >
                ...
                <div className="w-[36px] h-[36px] rounded-[10px] bg-white dark:bg-[#eef0f3] shadow-sm dark:shadow-none p-[3px] flex items-center justify-center overflow-hidden pointer-events-none">
                    {!iconError ? (
                        <img
                            src={`/icons/${link.icon}`}
                            alt={link.name}
            ...
            <span className="mt-1 text-[9px] font-semibold text-slate-700 dark:text-white/[0.82] text-center w-[52px] leading-[1.05] line-clamp-2">
                {link.name}
            </span>
        </div>
    );
